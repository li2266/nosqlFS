...
struct head_node * find_file_by_path(const char * path) {
    ...
    while (mongoc_cursor_next(cursor, &result)) {
        list_append(head, bson_copy(result));
...
struct head_node * find(char * name, const char * value, char * collection_name, char * sort_by, int sort) {
    ...
    filter = bson_new();
    if (value != NULL) {
        BSON_APPEND_UTF8(filter, "xattr", value);
    }
    if (sort_by != NULL) {
        opts = BCON_NEW("sort", "{", BCON_UTF8(sort_by), BCON_INT32(sort), "}");
        cursor = mongoc_collection_find_with_opts(_get_collection("nosqlFS", collection_name), filter, opts, NULL);
    } else {
        cursor = mongoc_collection_find(_get_collection("nosqlFS", collection_name), MONGOC_QUERY_NONE, 0, 0, 0, filter, NULL, NULL);
    }
